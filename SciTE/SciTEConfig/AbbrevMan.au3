...
Func _AbbrevManager()
	...
	#region ; Paths
		...
		Local $sUser_KeyWordPath = @UserProfileDir & "\au3.keywords.user.abbreviations.properties"
		Local $sSciTE_AbbrevPath = @UserProfileDir & "\au3abbrev.properties"
		Local $sUser_AbbrevPath = @UserProfileDir & "\au3userabbrev.properties"
		...
		If StringInStr(@ScriptDir, @ProgramFilesDir) Then
			...
			If Not FileExists(@AppDataDir & "\AbbrevMan") Then DirCreate(@AppDataDir & "\AbbrevMan")
			$sUser_BackUpPath = @AppDataDir & "\AbbrevMan\Abbrev.txt"
		Else
