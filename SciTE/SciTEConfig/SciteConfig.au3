...
#AutoIt3Wrapper_Res_Fileversion=1.6.11.1
...
#AutoIt3Wrapper_Res_Field=Credits|Melba23 - Abbrev and Calltip Managers and testing.
#AutoIt3Wrapper_Res_Field=Compile Date|%date% %time%
#AutoIt3Wrapper_Run_After=copy "%in%" "..\..\Programs_Updates\SciTEConfig\*.*"
#AutoIt3Wrapper_Run_After=copy "AbbrevMan.au3" "..\..\Programs_Updates\SciTEConfig\*.*"
#AutoIt3Wrapper_Run_After=copy "Scite_Reload_Props.au3" "..\..\Programs_Updates\SciTEConfig\*.*"
#AutoIt3Wrapper_Run_After=copy "StringSize.au3" "..\..\Programs_Updates\SciTEConfig\*.*"
#AutoIt3Wrapper_Run_After=copy "Get_AU3_RegistrySettings.au3" "..\..\Programs_Updates\SciTEConfig\*.*"
#AutoIt3Wrapper_Run_After=copy "UCTMan.au3" "..\..\Programs_Updates\SciTEConfig\*.*"
...
For $x = 1 To $CMDLINE[0]
	...
	Select
		Case $T_Var = "/Installer"
			; Process to be ran by the installer to preset some settings in SciTEUser.Properties
			; Get content SciTEUser and ensure we are on a newline at the end of the content
			$SciTEUser_Content = StringStripWS(FileRead(@UserProfileDir & "\SciTEUser.properties"), 3)
...
If Not FileExists($SciTE_Dir) Then
	...
	Exit
EndIf
; Determine UserDirectory
If @OSType = "WIN32_WINDOWS" Then
	$UserDir = $SciTE_Dir
Else
	If EnvGet("SciTE_HOME") <> "" And FileExists(EnvGet("SciTE_HOME")) Then
		$UserDir = EnvGet("SciTE_HOME")
	Else
		$UserDir = @UserProfileDir
	EndIf
EndIf
; Get content SciTEUser and ensure we are on a newline at the end of the content
Global $au3Prop = $UserDir & "\SciTEUser.properties"
...
Global $FontSize = "10"
Opt("GUICoordMode", 1)
Global $hSciTEConfig_GUI = GUICreate("SciTE Config for AutoIt3. ver:" & $VERSION, 410, 520, Default, Default, Default, $WS_EX_TOPMOST)
Global $NeedsSave = 0
...
Global $H_ProperCase
Global $H_Display_Inline_Errors
...
Global $BaseX = 30
Global $BaseY = 40
Global $h_tab = GUICtrlCreateTab(10, 10, 390, 450)
Global $h_tab0 = GUICtrlCreateTabItem("General Settings")
...
Global $h_Edit = GUICtrlCreateRadio('Edit', $BaseX + 190, $BaseY + 15, 50, 15)
Global $h_Run = GUICtrlCreateRadio('Run', $BaseX + 190, $BaseY + 35, 50, 15)
...
Global $h_Backups = GUICtrlCreateInput($Backups, $BaseX + 245, $BaseY + 12, 25, 20)
$H_ProperCase = GUICtrlCreateCheckbox("Auto Propercase Functions and Keywords.", $BaseX + 10, $BaseY + 30)
$H_Display_Inline_Errors = GUICtrlCreateCheckbox("Display inline errors in the Editor.", $BaseX + 10, $BaseY + 50)
...
GUICtrlCreateLabel($AutoIT3_Dir, $BaseX + 10, $BaseY + 18, 400, 20, $SS_LEFTNOWORDWRAP)
...
GUICtrlCreateLabel("User Includes:", $BaseX + 10, $BaseY + 35, 70, 20)
Global $h_UserIncludeDirectory = GUICtrlCreateInput($UserIncludeDirectory, $BaseX + 10, $BaseY + 55, 325, 20)
Global $h_UserIncludeDirectory_Button = GUICtrlCreateButton("...", $BaseX + 340, $BaseY + 55, 20, 20)
;
$BaseY = 290
...
;
; Color Tab
Global $h_tab1 = GUICtrlCreateTabItem("Color Settings")
$BaseY = 60
;determine current loaded definition files for SciTE
;$BaseY = 148
Global $H_Background_Label = GUICtrlCreateLabel("Background Color:" & StringLower($Background_Color), $BaseX + 5, $BaseY - 20, 150, 20)
Global $H_CaretLine_Label = GUICtrlCreateLabel("Caret line Color:" & StringLower($CaretLine_Color), $BaseX + 5, $BaseY, 150, 20)
Global $H_Selection_Label = GUICtrlCreateLabel("Selected Color", $BaseX + 5, $BaseY + 20, 150, 20)
Global $H_Alpha_Label = GUICtrlCreateLabel("Alpha", $BaseX + 170 + 84 + 55, $BaseY + 22, 100, 20)
Global $H_Background_Color = GUICtrlCreateButton("BackGround", $BaseX + 170, $BaseY - 20, 80, 20)
Global $H_CaretLine_Color = GUICtrlCreateButton("CaretLine", $BaseX + 170, $BaseY, 80, 20)
Global $H_Selection_Color = GUICtrlCreateButton("Fore", $BaseX + 170, $BaseY + 20, 40, 20)
Global $H_Selection_BkColor = GUICtrlCreateButton("Back", $BaseX + 170 + 40, $BaseY + 20, 40, 20)
Global $H_Selection_Alpha = GUICtrlCreateInput($Selection_Alpha, $BaseX + 170 + 84, $BaseY + 20, 50, 20)
...
Global $file_au3properties, $h_treeview, $h_tab2, $Tool_Num, $Tool_Desc_Org, $Tool_Desc_Cur
...
If FileExists($AutoIT3_Dir & '\SciTE\Properties\au3.properties') Then
	...
	Global $comment = '#~ '
	Global $SciTETools = SciteToolsFileRead($file_au3properties)
	If @error Then
	...
	Else
		...
		GUICtrlCreateLabel("Select items to enable or disable from Scite's Tools menu", 40, 35, 330)
...
EndIf
;
Global $h_tab3 = GUICtrlCreateTabItem("Other Tools")
...
Global $H_RunRegistry_Check = GUICtrlCreateButton("Run AutoIt3 Registry check", 20, 120, 200, 30)
;
GUICtrlCreateTabItem("") ; end tabitem definition
Global $Status_Msg = GUICtrlCreateLabel("", 10, 465, 390, 17, $SS_SUNKEN)
GUICtrlSetBkColor(-1, 0xC0C0C0)
Global $H_Update = GUICtrlCreateButton("Save+Apply", 10, 490, 70)
GUICtrlSetState(-1, $GUI_DISABLE)
GUICtrlSetTip(-1, "Show changes in SciTE and save to SciTEUser.properties")
Global $H_NewScheme = GUICtrlCreateButton("New Scheme", 90, 490, 70)
GUICtrlSetTip(-1, "Select a new Color&Font Scheme.")
GUICtrlSetState($H_NewScheme, $GUI_HIDE)
Global $H_Exit = GUICtrlCreateButton("Exit", 170, 490, 50)
...
While 1
	...
	Select
		...
		Case $Msg = $H_Update
			Update_SciTE_User()
			Reload_Config()
			Update_status("Updated Configuration and config reloaded.")
			$NeedsSave = 0
;~ 			MsgBox(262144 + 48, "SciTE config", "Updated Configuration")
		...
		Case $Msg = $h_Edit
			...
			Update_status('Updated default to Open in the registry.')
			;
		Case $Msg = $h_Run
			...
			Update_status('Updated default to Run in the registry.')
		Case $Msg = $H_ProperCase
			$NeedsSave = 1
		Case $Msg = $H_Display_Inline_Errors
			$NeedsSave = 1
			;
		Case $Msg = $h_Backups
			$NeedsSave = 1
		...
		Case $Msg = $h_UserIncludeDirectory_Button
			GUISetState(@SW_HIDE)
			Global $NewDir = FileSelectFolder("Select the User Include directory:", "", 4, GUICtrlRead($h_UserIncludeDirectory))
			If @error = 0 Then
		...
		Case $Msg = $H_Background_Color
			...
			$NeedsSave = 1
			;
		Case $Msg = $H_CaretLine_Color
			$tempcolor = SelectColor($CaretLine_Color)
	...
	; Set the save button
	If $NeedsSave Then
		If BitAND(GUICtrlGetState($H_Update), $GUI_DISABLE) = $GUI_DISABLE Then GUICtrlSetState($H_Update, $GUI_ENABLE)
	Else
		If BitAND(GUICtrlGetState($H_Update), $GUI_ENABLE) = $GUI_ENABLE Then GUICtrlSetState($H_Update, $GUI_DISABLE)
	EndIf
	; Display Status update
	If $NeedsSave <> $check_NeedsSave Then
		Update_status('Update made to settings. Press Save+Apply to store into SciTEUser.')
		$check_NeedsSave = $NeedsSave
	EndIf
WEnd
Exit
#EndRegion ; GUI
...
;
Func Reset_Status()
	GUICtrlSetBkColor($Status_Msg, 0xC0C0C0)
...
Func SelectNewScheme()
	...
	If $search = -1 Then
		...
		Update_status('There are no *.SciTEConfig files available!')
;~ 		MsgBox(262144, 'SciTE Config', 'There are no *.SciTEConfig files available.')
	...
	Local $H_au3PropScheme = FileOpen(@ScriptDir & "\" & $SchemeFile, 0)
	...
	If $SchemeFile <> "[Last_User_setting].SciTEConfig" Then
		If StringInStr($SaveUserSettings, "# Created by SciTEConfig") Then
			Local $H_au3PropScheme_User = FileOpen(@ScriptDir & "\[Last_User_setting].SciTEConfig", 2)
...
Func Update_SciTE_User()
	...
	If GUICtrlRead($h_Backups) > 0 Then
		Update_SciTE_UserContent("Update", "backup.files", "" & GUICtrlRead($h_Backups))
	EndIf
	;propercase info
	...
	If GUICtrlRead($H_Display_Inline_Errors) = 1 Then
...
;~ 	EndIf
	...
	If $CaretLine_Color <> "" And $CaretLine_Color <> "0XFFFFFF" Then
		Update_SciTE_UserContent("Update", "caret.line.back", "#" & StringTrimLeft($CaretLine_Color, 2))
	EndIf
...
;~ 				ConsoleWrite("! Key not Found: " & $aKeyValue & @CRLF)
		...
		Case "Save"
			...
			FileRecycle($UserDir & "\SciTEUser.properties")
			Local $rc = FileWrite($UserDir & "\SciTEUser.properties", StringTrimRight($SciTEUser_Content, 2))
			If Not $rc Then ConsoleWrite("! Unable to write changes to " & $UserDir & "\SciTEUser.properties" & @CRLF)
	EndSwitch
EndFunc   ;==>Update_SciTE_UserContent
;
Func Update_status($NewText)
	AdlibRegister("Reset_Status", 2000)
	GUICtrlSetBkColor($Status_Msg, 0x00FF00)
	GUICtrlSetData($Status_Msg, $NewText)
...
Func Update_Window()
	...
	If $ErrorInline = 1 Then
		GUICtrlSetState($H_Display_Inline_Errors, 1)
	Else
		GUICtrlSetState($H_Display_Inline_Errors, 4)
	EndIf
	If $SYN_Font_Mono_ON = 1 Then
		GUICtrlSetState($H_Syn_Mono, 1)
		GUICtrlSetState($H_Syn_Prop, 4)
