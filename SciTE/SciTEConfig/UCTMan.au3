...
Func _CallTipManager()
	...
	#region ; File Paths
		...
		Local $sCallTipPath = @UserProfileDir & "\au3.user.calltips.api"
		Local $sKeyWordPath = @UserProfileDir & "\au3.userudfs.properties"
		...
		If StringInStr(@ScriptDir, @ProgramFilesDir) Then
			...
			If Not FileExists(@AppDataDir & "\UCTMan") Then DirCreate(@AppDataDir & "\UCTMan")
			$sUserPath = @AppDataDir & "\UCTMan\UCT.txt"
		Else
...
#endregion ; Reinstall, Show, View, Delete Functions
