...
Const $CM_REENUMERATE_RETRY_INSTALLATION = 0x00000002
Const $CM_REENUMERATE_ASYNCHRONOUS = 0x00000004
Const $CM_REENUMERATE_BITS = 0x00000007
; ====================================================================================
...
; #### Flags for _CM_Parse_PCI_Device.Flag ####
; =====================================================================================================
Const $PARSE_PCI_DEVICE_VEN = 1
Const $PARSE_PCI_DEVICE_DEVICE = 2
Const $PARSE_PCI_DEVICE_SUB_SYSTEM = 3
Const $PARSE_PCI_DEVICE_REVISION = 4
Const $PARSE_PCI_DEVICE_DEV_CLASS = 5
Const $PARSE_PCI_DEVICE_BUS_NUMBER = 6
Const $PARSE_PCI_DEVICE_DEV_NUMBER = 7
Const $PARSE_PCI_DEVICE_FUNCTION = 8
; =====================================================================================================
...
Func _SetupDiEnumDeviceInfo($hDevs, $iIndex, ByRef $tSP_DEVINFO_DATA)
	...
	If Not IsDllStruct($tSP_DEVINFO_DATA) Then
		...
		If @OSArch='x64' Then
			DllStructSetData($tSP_DEVINFO_DATA, "Size", 32)
		Else
...
;		:	Func _EnumDeviceNotifyProgressCallback($iIndex, $sDeviceID, $hDevInst, $sDescr)
...
Func _CM_Enum_Device_Info_Notify_Progress($vClassGuid, $sEnumerator = "", $sFunction = "")
	Local $aResult[1][4], $sDescr, $sName, $tDevInfo, $hDevs, $iFlags

	If ($vClassGuid = "") Then
		$iFlags = BitOR($DIGCF_PRESENT, $DIGCF_ALLCLASSES)
	Else
		$iFlags = $DIGCF_PRESENT
	EndIf

	$hDevs = _SetupDiGetClassDevs($iFlags, $vClassGuid, $sEnumerator)
	While _SetupDiEnumDeviceInfo($hDevs, $aResult[0][0], $tDevInfo)
		$sDescr = _SetupDiGetDeviceRegistryProperty($hDevs, $tDevInfo, 0)
		$sName = _SetupDiGetDeviceRegistryProperty($hDevs, $tDevInfo, 0xC)
		If $sName <> "" Then $sDescr = $sName
		...
		$aResult[$aResult[0][0]][2] = DllStructGetData($tDevInfo, "DevInst")
		If $sFunction <> "" Then
			$aResult[$aResult[0][0]][3] = Call($sFunction, $aResult[0][0], _
				$aResult[$aResult[0][0]][1], $aResult[$aResult[0][0]][2], $aResult[$aResult[0][0]][0])
		EndIf
	WEnd
	_CM_Assign_Var($tDevInfo, 0, @error)
	Return SetError(@error, _SetupDiDestroyDeviceInfoList($hDevs), $aResult)
...
; #### FUNCTION ####
; =====================================================================================================
; Name	: _CM_Parse_PCI_Device
; Description	: This function parses a PCI device, such as sub-system, bus number, or function.
; Parameter(s)	: $hDevInst - Supplies the handle to a PCI device.
;		: $iFlag - Information class required, see _CM_Parse_PCI_Device.Flag for a value.
; Return values	: Information requested is returned if succeeds, in string format. Else returns NULL and sets @error.
; Author	: Pusofalse
; =====================================================================================================
Func _CM_Parse_PCI_Device($hDevInst, $iFlag)
	Local $aResult, $sDeviceId

	If _CM_Get_DevNode_Registry_Property($hDevInst, 0x17) <> "PCI" Then
		Return SetError(87, 0, "")
	EndIf

	Switch $iFlag
	Case $PARSE_PCI_DEVICE_VEN
		$sDeviceId = _CM_Get_Device_Id($hDevInst)
		$aResult = StringRegExp($sDeviceId, "(?i)VEN_([0-9A-F]+)&", 3)
	Case $PARSE_PCI_DEVICE_DEVICE
		$sDeviceId = _CM_Get_Device_Id($hDevInst)
		$aResult = StringRegExp($sDeviceId, "(?i)DEV_([0-9A-F]+)&", 3)
	Case $PARSE_PCI_DEVICE_SUB_SYSTEM
		$sDeviceId = _CM_Get_Device_Id($hDevInst)
		$aResult = StringRegExp($sDeviceId, "(?i)SUBSYS_([0-9A-F]+)&", 3)
		If IsArray($aResult) Then
			$aResult[0] = StringRegExpReplace($aResult[0], "(....)(....)", "\2-\1")
		EndIf
	Case $PARSE_PCI_DEVICE_REVISION
		$sDeviceId = _CM_Get_Device_Id($hDevInst)
		$aResult = StringRegExp($sDeviceId, "(?i)REV_([0-9A-F]+)\\", 3)
	Case $PARSE_PCI_DEVICE_DEV_CLASS
		$aResult = _CM_Get_DevNode_Registry_Property($hDevInst, 2)
		While StringRight($aResult, 1) = @LF
			$aResult = StringTrimRight($aResult, 1)
		WEnd
		$aResult = StringSplit($aResult, @LF)
		If ($aResult[0] < 4) Then
			$aResult = 0
		Else
			$aResult = StringRegExp($aResult[4], "(?i)_([0-9A-F]+)$", 3)
		EndIf
	Case $PARSE_PCI_DEVICE_BUS_NUMBER, $PARSE_PCI_DEVICE_DEV_NUMBER, $PARSE_PCI_DEVICE_FUNCTION
		$aResult = _CM_Get_DevNode_Registry_Property($hDevInst, 0xE)
		$aResult = StringSplit($aResult, ", ", 1)
		If ($aResult[0] <> 3) Then
			$aResult[0] = ""
		Else
			$aResult = StringRegExp($aResult[$iFlag - 5], "(\d+)$", 3)
		EndIf
	Case Else
		Return SetError(87, 0, "")
	EndSwitch
	...
	If IsArray($aResult) And ($aResult[0] <> "") Then
		Return $aResult[0]
	Else
		Return SetError($CR_NO_SUCH_VALUE, 0, "")
	EndIf
EndFunc	;==>_CM_Parse_PCI_Device

Func _CM_Format_PCI_Device_Class($sClassCode)
	Local $sText

	Switch $sClassCode
	Case "0200"
		$sText = "Ethernet Controller"
	Case "0604"
		$sText = "PCI/PCI Bridge"
	Case "0C03"
		$sText = "USB Controller"
	Case "0403"
		$sText = "High Defination Audio"
	Case "0101"
		$sText = "IDE Controller"
	Case "0C05"
		$sText = "SMBus Controller"
	Case "0601"
		$sText = "PCI/ISA Bridge"
	Case "0600"
		$sText = "Host/PCI Bridge"
	Case "0280"
		$sText = "Network Controller"
	Case "0300"
		$sText = "VGA Display Controller"
	Case "0C00"
		$sText = "FireWire Controller"
	Case "0607"
		$sText = "PCI/CardBus Bridge"
	Case "0180"
		$sText = "Mass Storage Controller"
	EndSwitch
	Return $sText
EndFunc	;==>_CM_Format_PCI_Device_Class

Func _SetupDiGetSelectedDevice($hDevs, ByRef $tSP_DEVINFO_DATA)
	...
	EndIf
	$pBuffer = DllStructGetPtr($tSP_DEVINFO_DATA)

	$iResult = DllCall($SETUPAPI_DllHandle, "int", "SetupDiGetSelectedDevice", _
			"hWnd", $hDevs, "ptr", $pBuffer)
	If ($iResult[0] = 0) Then $tSP_DEVINFO_DATA = 0
	Return SetError(_CM_Get_Last_Error(), 0, $iResult[0])
EndFunc	;==>_SetupDiGetSelectedDevice

Func _SetupDiRestartDevice($hDevs, $pSP_DEVINFO_DATA)
	Local $tBuffer

	$tBuffer = DllStructCreate($tagSP_PROPCHANGE_PARAMS)
	DllStructSetData($tBuffer, "Size", 8)
	DllStructSetData($tBuffer, "DIFCode", $DIF_PROPERTYCHANGE)
	DllStructSetData($tBuffer, "State", $DICS_STOP)
	DllStructSetData($tBuffer, "Scope", $DICS_FLAG_CONFIGSPECIFIC)

	If Not _SetupDiSetClassInstallParams($hDevs, $pSP_DEVINFO_DATA, $tBuffer, 20) Then
		Return SetError(@error, _SetupDiApiBufferFree($tBuffer), 0)
	EndIf
	If Not _SetupDiChangeState($hDevs, $pSP_DEVINFO_DATA) Then
		Return SetError(@error, _SetupDiApiBufferFree($tBuffer), 0)
	EndIf

	DllStructSetData($tBuffer, "State", $DICS_START)
	If Not _SetupDiSetClassInstallParams($hDevs, $pSP_DEVINFO_DATA, $tBuffer, 20) Then
		Return SetError(@error, _SetupDiApiBufferFree($tBuffer), 0)
	EndIf
	If Not _SetupDiChangeState($hDevs, $pSP_DEVINFO_DATA) Then
		Return SetError(@error, _SetupDiApiBufferFree($tBuffer), 0)
	EndIf
	Return SetError(@error, _SetupDiApiBufferFree($tBuffer), 1)
EndFunc	;==>_SetupDiRestartDevice

Func _SetupDiGetClassInstallParams($hDevs, $pSP_DEVINFO_DATA, ByRef $pSP_ClASSINST_PARAMS)
	Local $iResult, $tBuffer

	If IsDllStruct($pSP_DEVINFO_DATA) Then
		$pSP_DEVINFO_DATA = DllStructGetPtr($pSP_DEVINFO_DATA)
	EndIf
	$iResult = DllCall($SETUPAPI_DllHandle, "int", "SetupDiGetClassInstallParams", _
			"hWnd", $hDevs, "ptr", $pSP_DEVINFO_DATA, "ptr", 0, "dword", 0, _
			"dword*", 0)
	If ($iResult[5] = 0) Then Return SetError(_CM_Get_Last_Error(), 0, 0)

	$pSP_CLASSINST_PARAMS = _CM_Heap_Alloc($iResult[5])
	$tBuffer = DllStructCreate($tagSP_CLASSINSTALL_HEADER, $pSP_CLASSINST_PARAMS)
	DllStructSetData($tBuffer, "Size", 8)

	$iResult = DllCall($SETUPAPI_DllHandle, "int", "SetupDiGetClassInstallParams", _
			"hWnd", $hDevs, "ptr", $pSP_DEVINFO_DATA, "ptr", $pSP_CLASSINST_PARAMS, _
			"dword", $iResult[5], "ptr", 0)
	If ($iResult[0] = 0) Then
		_CM_Assign_Var($tBuffer, 0, 0, _CM_Get_Last_Error(), _CM_Heap_Free($pSP_CLASSINST_PARAMS))
		Return SetError(@error, _CM_Assign_Var($pSP_ClASSINST_PARAMS, 0), 0)
	EndIf
	Return SetError(_CM_Get_Last_Error(), _CM_Assign_Var($tBuffer, 0), 1)
EndFunc	;==>_SetupDiGetClassInstallParams

Func _CM_Is_Device_Stopped($hDevInst)
	Local $sDeviceId, $sRegKey, $iCSFlags

	$iCSFlags = _CM_Get_DevNode_Status($hDevInst)
	If (@error) Then Return SetError(@error, -1, 0)

	If BitAnd($iCSFlags, $DN_DRIVER_LOADED) Or BitAnd($iCSFlags, $DN_STARTED) Then
		Return 0
	EndIf

	$sDeviceId = _CM_Get_Device_ID($hDevInst)

	$sRegKey = $REGKEY_HARDWARE_CS002_001 & $sDeviceId
	$iCSFlags = RegRead($sRegKey, "CSConfigFlags")
	If (@error) Then Return SetError(@error, -1, 0)
	Return BitAnd($iCSFlags, $CSCONFIGFLAG_DO_NOT_START)
EndFunc	;==>_CM_Is_Device_Stopped


Func _CM_Lock_File($hFile, $iOffset64, $iBytesLock64, $fLock = 1)
	Local $iResult

	If $fLock Then
		$iResult = DllCall("Kernel32.dll", "bool", "LockFile", "handle", $hFile, _
				"int64", $iOffset64, "int64", $iBytesLock64)
	Else
		$iResult = DllCall("Kernel32.dll", "bool", "UnlockFile", "handle", $hFile, _
				"int64", $iOffset64, "int64", $iBytesLock64)
	EndIf
	Return SetError(_CM_Get_Last_Error(), 0, $iResult[0])
EndFunc	;==>_CM_Lock_File

Func _CM_Set_Information_File($hFile, $iInfoClass, $vBuffer, $iBuffer, $sBufferType = "ptr")
	Local $iResult
	$iResult = DllCall("Ntdll.dll", "dword", "NtSetInformationFile", "handle", $hFile, _
			"ptr", _CM_Heap_Alloc(32), $sBufferType, $vBuffer, "ulong", $iBuffer, _
			"ulong", $iInfoClass)
	_CM_Heap_Free($iResult[2])
	Return SetError(_LsaNtStatusToWinError($iResult[0]), 0, $iResult[0] = 0)
EndFunc	;==>_CM_Set_Information_File

Func _CM_Set_File_Pointer($hFile, $iPointer, $iMethod = 0, $iHighOrder = 0)
	Local $iResult, $sBufferType = "ptr"

	If $iHighOrder Then $sBufferType = "dword*"

	$iResult = DllCall("Kernel32.dll", "dword", "SetFilePointer", "handle", $hFile, _
			"dword", $iPointer, $sBufferType, $iHighOrder, _
			"dword", $iMethod)
	Return SetError(_CM_Get_Last_Error(), $iResult[3], $iResult[0])
EndFunc	;==>_CM_Set_File_Pointer

Func _CM_Query_Information_File($hFile, $iInfoClass, $vBuffer, $iSizeofBuffer, $sBufferType = "ptr")
	Local $iResult

	$iResult = DllCall("Ntdll.dll", "dword", "NtQueryInformationFile", "handle", $hFile, _
			"ptr", _CM_Heap_Alloc(32), $sBufferType, $vBuffer, "ulong", $iSizeofBuffer, _
			"long", $iInfoClass)
	If @error Then Return SetError(87, 0, 0)
	...
	If ($sBufferType = "ptr") Then
		Return SetError(_LsaNtStatusToWinError($iResult[0]), 0, $iResult[0] = 0)
	Else
		Return SetError(_LsaNtStatusToWinError($iResult[0]), 0, $iResult[3])
	EndIf
EndFunc	;==>_CM_Query_Information_File
