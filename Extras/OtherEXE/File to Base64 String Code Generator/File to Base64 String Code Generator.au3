...
#AutoIt3Wrapper_OutFile=File to Base64 String Code Generator.exe
...
#pragma compile(ProductName, 文件转换为Base64字符 代码生成器)
#pragma compile(LegalCopyright, UEZ 2011-2014)
...
Global Const $title = "文件转换为Base64字符代码生成器 "
_GDIPlus_Startup()
...
GUISetFont(8 * $aDPI[0], 400, 0, "微软雅黑")
...
Global $ini_file = @ScriptDir & "\FiletoBase64StringCodeGenerator.ini"
Global $hFile_Ini
If Not FileExists($ini_file) Then
	$hFile_Ini = FileOpen($ini_file, 1)
	FileWriteLine($hFile_Ini, ";某些颜色值: " & $sBGColors)
	FileClose($hFile_Ini)
...
Global Const $idLabel_File = GUICtrlCreateLabel("选择文件:", 8, 16, 70, 20)
GUICtrlSetFont(-1, 10 * $aDPI[0], 400, 0, "微软雅黑")
GUICtrlSetTip(-1, "选择您想转换为base64字符串的文件（可多选）", "", 0, 1)
Global Const $idButton_File = GUICtrlCreateButton("[&B]浏览", 80, 12, 75, 25)
GUICtrlSetFont(-1, 9 * $aDPI[0], 400, 0, "微软雅黑")
GUICtrlSetTip(-1, "选择您想转换为base64字符串的文件（可多选）", "", 0, 1)
Global Const $idInput_File = GUICtrlCreateInput("", 160, 14, 473, 22, $ES_READONLY)
GUICtrlSetFont(-1, 10 * $aDPI[0], 400, 0, "微软雅黑")
GUICtrlSetState(-1, $GUI_DROPACCEPTED)
Global Const $idLabel_VarName = GUICtrlCreateLabel("变量 / 函数名称:", 8, 49, 155, 20)
GUICtrlSetFont(-1, 10 * $aDPI[0], 400, 0, "微软雅黑")
GUICtrlSetTip(-1, "请输入不以$开头的变量或函数名称" & @LF & "在多选模式下将使用文件名!", "", 0, 1)
Global Const $idInput_Var = GUICtrlCreateInput("Base64String", 164, 46, 221, 22)
GUICtrlSetFont(-1, 10 * $aDPI[0], 400, 0, "微软雅黑")
GUICtrlSetTip(-1, "请输入不以$开头的变量或函数名称" & @LF & "在多选模式下将使用文件名!", "", 0, 1)
;~ GUICtrlSetState(-1, $GUI_DROPACCEPTED)
Global Const $idCheckbox_ContFuncName = GUICtrlCreateCheckbox("创建包含顺序号的函数名称", 165, 76, 220, 16, Default, $WS_EX_LAYOUTRTL)
GUICtrlSetFont(-1, 8 * $aDPI[0], 400, 0, "微软雅黑")
GUICtrlSetTip(-1, "当选择多个文件时,创建包含顺序号的函数名"&@LF&"称如Func1、func2的函数名称以便脚本更容易处理~", "", 0, 1)
Global Const $idButton_Convert = GUICtrlCreateButton("生成", 8, 102, 83, 43, $BS_BITMAP)
GUICtrlSetFont(-1, 21 * $aDPI[0], 400, 0, "微软雅黑")
GUICtrlSetTip(-1, "转换任意文件到base64字符串并生成相关调用函数", "", 0, 1)
...
_GUICtrlRichEdit_SetFont($hRichEdit, 9 * $aDPI[0], "微软雅黑")
...
Global Const $idButton_Save = GUICtrlCreateButton("[&S]保存", 100, 102, 75, 43)
GUICtrlSetFont(-1, 9 * $aDPI[0], 400, 0, "微软雅黑")
GUICtrlSetTip(-1, "保存生成代码至文件(AU3 后缀文件)", "", 0, 1)
Global Const $idButton_Clipboard = GUICtrlCreateButton("[&P]复制到剪切板", 180, 102, 93, 43)
GUICtrlSetFont(-1, 9 * $aDPI[0], 400, 0, "微软雅黑")
GUICtrlSetTip(-1, "左键：复制到剪切板" & @LF & "右键: 复制代码到剪切板并直接粘贴至SciTE编辑器", "", 0, 1)


Global Const $idCheckbox_Compression = GUICtrlCreateCheckbox("启用压缩", 304, 102, 81, 17, Default, $WS_EX_LAYOUTRTL)
GUICtrlSetFont(-1, 8 * $aDPI[0], 400, 0, "微软雅黑")
If $LZNT_compression_strength < 2 Then GUICtrlSetState(-1, $GUI_CHECKED)
GUICtrlSetTip(-1, "启用内置LZNT或LZMAT压缩算法" & @LF & "点击右键设置LZNT压缩强度" & @LF & ".默认: 高压缩(速度较慢)", "", 0, 1)

Global Const $idCheckbox_DecompFunction = GUICtrlCreateCheckbox("添加解压缩函数", 275, 129, 110, 17, Default, $WS_EX_LAYOUTRTL)
GUICtrlSetFont(-1, 8 * $aDPI[0], 400, 0, "微软雅黑")
If $DeCompFct Then GUICtrlSetState(-1, $GUI_CHECKED)
GUICtrlSetTip(-1, "在生成代码的同时添加相应解压缩函数", "", 0, 1)
Global Const $idGroup_Statistics = GUICtrlCreateGroup("文件信息", 392, 40, 241, 105)
Global Const $idLabel_Length = GUICtrlCreateLabel("源文件大小:", 400, 60, 59, 18)
GUICtrlSetTip(-1, "文件大小", "", 0, 1)
Global Const $idLabel_Lenght_n = GUICtrlCreateLabel(StringFormat("%.2f %s", 0, "kb"), 466, 60, 96, 18)
Global Const $idLabel_Length_C = GUICtrlCreateLabel("新文件大小:", 400, 90, 66, 18)
GUICtrlSetTip(-1, "压缩后文件大小", "", 0, 1)
Global Const $idLabel_Length_C_n = GUICtrlCreateLabel(StringFormat("%.2f %s", 0, "kb"), 466, 90, 88, 18)
Global Const $idLabel_ScriptLength = GUICtrlCreateLabel("脚本文件大小:", 400, 120, 66, 18)
GUICtrlSetTip(-1, "脚本文件大小", "", 0, 1)
Global Const $idLabel_ScriptLength_n = GUICtrlCreateLabel(StringFormat("%.2f %s", 0, "kb"), 466, 120, 88, 18)
GUICtrlSetTip(-1, "生成的AutoIt代码文件大小", "", 0, 1)
Global Const $idLabel_Saved = GUICtrlCreateLabel("节省:", 539, 90, 38, 18)
GUICtrlSetTip(-1, "压缩优化大小", "", 0, 1)
Global Const $idLabel_SavedPerc = GUICtrlCreateLabel(StringFormat("%05.2f %s", 0, "%"), 579, 90, 46, 18)
GUICtrlSetFont(-1, 8 * $aDPI[0], 800, 0, "微软雅黑")
GUICtrlSetTip(-1, "该项值越高则压缩效果越好~!", "", 0, 1)
Global Const $idLabel_MS = GUICtrlCreateLabel("多选模式:", 539, 60, 57, 14)
GUICtrlSetTip(-1, "如果多选模式被设置为True，则请点击左键以显示文件信息", "", 0, 1)
...
_GUICtrlMenu_InsertMenuItem($hQMenu_Sub1, 0, "标准", $id_LZNT_Standard)
_GUICtrlMenu_InsertMenuItem($hQMenu_Sub1, 1, "高", $id_LZNT_High)

Global Const $hQMenu = _GUICtrlMenu_CreatePopup()
_GUICtrlMenu_InsertMenuItem($hQMenu, 0, "LZNT压缩强度", $id_LZNT, $hQMenu_Sub1)
_GUICtrlMenu_InsertMenuItem($hQMenu, 1, "LZMAT压缩", $id_LZMAT)
...
_GUICtrlMenu_AppendMenu($hMenu, $MF_STRING, $id_About, "关于")
...
_GUICtrlMenu_InsertMenuItem($hPopupMenu, 0, "复制", $id_Copy)
...
_GUICtrlRichEdit_SetText($hRichEdit, "准备就绪.")
...
While 1
	Switch GUIGetMsg()
		...
		Case $idButton_File
			Load_File()
		Case $idButton_Save
			If _GUICtrlRichEdit_GetText($hRichEdit) = "" Or _GUICtrlRichEdit_GetText($hRichEdit) = "准备就绪." Then
				MsgBox(16 + 262144, "错误", "没有可供保存的内容!" & @LF & @LF & "在点击此按钮前请先将文件进行转换!", 30, $hGUI)
			Else
				Save_File()
			EndIf
		Case $idButton_Clipboard
			$text = _GUICtrlRichEdit_GetText($hRichEdit)
			If $text = "" Or $text = "准备就绪." Then
				MsgBox(16 + 262144, "错误", "没有可供保存的内容!" & @LF & @LF & "在点击此按钮前请先将文件进行转换!", 30, $hGUI)
			Else
				If Not ClipPut($text) Then $err = True
				If $err Then
					MsgBox(16 + 262144, "错误", "未能将脚本文件复制到剪切板!", 30, $hGUI)
				Else
					MsgBox(64 + 262144, "提示", "脚本被复制到剪切板!", 30, $hGUI)
				EndIf
			EndIf
	...
	Switch $aPos[4]
		...
		Case $idButton_Clipboard
			If $aPos[3] Then
				...
				$text = _GUICtrlRichEdit_GetText($hRichEdit)
				If $hSciTE And $text <> "准备就绪." And $text <> "" Then
					ClipPut($text)
					WinActivate($hSciTE)
					Sleep(500)
					Send("^v")
				...
				Else
					If Not IsHWnd($hSciTE) Then
						MsgBox(16 + 262144, "错误", "未检测到SciTE编辑器窗口!", 30, $hGUI)
					Else
						MsgBox(16 + 262144, "错误", "没有可供保存的内容!" & @LF & @LF & "在点击此按钮前请先将文件进行转换!", 30, $hGUI)
					EndIf
				EndIf
...
Func Convert()
	If GUICtrlRead($idInput_File) = "" Then Return MsgBox(16 + 262144, "错误", "未选择文件!", 30, $hGUI)
	GUICtrlSetState($idButton_Convert, $GUI_DISABLE)
	GUISetState(@SW_DISABLE, $hGUI)
	GUICtrlSetColor($idLabel_SavedPerc, 0x000000)
	_GUICtrlRichEdit_SetText($hRichEdit, "正在转换文件为base64字符串，请稍后...")
	...
	$Script = ";以下代码由: 文件转换为Base64字符代码生成器汉化版By 虫子樱桃 生成"
	If $multiselect Then
		...
		If $VarName = "" And $bChkContFuncName Then
			...
			Return MsgBox(16 + 262144, "错误", "函数名为空!", 30, $hGUI)
		EndIf
		...
		If Not $compressed2 And $compression Then
			...
			ToolTip("压缩后的文件大小较原文件大!" & @LF & "所有文件压缩禁用!", DllStructGetData($tPoint, "X"), DllStructGetData($tPoint, "Y"), "警告", 2, 1)
			GUICtrlSendToDummy($dummy)
	...
	Else
		...
		If Not FileExists($file) Then
			_Enable_Ctrls($old_cursor)
			_GUICtrlRichEdit_SetText($hRichEdit, "准备就绪.")
			Return MsgBox(16 + 262144, "错误", '未找到文件：' & $file , 30, $hGUI)
		EndIf
		If FileGetSize($file) = 0 Then
			_Enable_Ctrls($old_cursor)
			_GUICtrlRichEdit_SetText($hRichEdit, "准备就绪.")
			Return MsgBox(16 + 262144, "错误", '"' & $file & '" 文件为空(0 kb)', 30, $hGUI)
		EndIf
		$VarName = StringRegExpReplace(GUICtrlRead($idInput_Var), "[^\w]", "")
		If $VarName = "" Then
			_Enable_Ctrls($old_cursor)
			Return MsgBox(16 + 262144, "	错误", "变量或函数名为空!", 30, $hGUI)
		EndIf
		...
		If $compression Then
			...
			Else
				...
				DllStructSetData($tPoint, "Y", $aCoord[1] + 12)
				_WinAPI_ClientToScreen($hGUI, $tPoint)
				ToolTip("生成大小教原文件大!" & @LF & "压缩禁用!", DllStructGetData($tPoint, "X"), DllStructGetData($tPoint, "Y"), "警告", 2, 1)
				GUICtrlSendToDummy($dummy)
...
Func Load_File($file = "")
	...
	ToolTip("")
	If $file = "" Then
		$file = FileOpenDialog("选择要转换为base64字符串的文件", "", "所有文件 (*.*)", 3 + 4, "", $hGUI)
		If @error Then Return MsgBox(64 + 262144, "警告", "文件选择操作被取消!", 20, $hGUI)
	EndIf
	_GUICtrlRichEdit_SetText($hRichEdit, "")
	Local $fSize, $c, $s
	...
	If Not @error Then
		...
		Else
			GUICtrlSetState($idInput_Var, $GUI_ENABLE)
		EndIf
		$sFiles = "已经选择" &UBound($aFiles) - 1 &"个文件！"  &@LF & "准备就绪!"
...
Func Save_File()
	Local $filename = FileSaveDialog("保存", "", "AutoIt脚本文件(*.au3)", 18, "", $hGUI)
	If @error Then Return MsgBox(48 + 262144, "警告", "保存操作被取消!", 30, $hGUI)
	If StringRight($filename, 4) <> ".au3" Then $filename &= ".au3"
	Local $hFile = FileOpen($filename, 2)
	If $hFile = -1 Then Return MsgBox(16 + 262144, "错误", "不能创建'" & $filename & "'!", 30, $hGUI)
	FileWrite($hFile, _GUICtrlRichEdit_GetText($hRichEdit))
	FileClose($hFile)
	Return MsgBox(64 + 262144, "提示", "生成的代码被成功保存至文件'" & $filename & "'!", 30, $hGUI)
EndFunc   ;==>Save_File

Func Display_Statistic($array)
	GUISetState(@SW_DISABLE, $hGUI)
	Local $w = 500, $h = 600
	Local $hGUI_stat = GUICreate("文件信息", $w, $h, -1, -1, Default, Default, $hGUI)
	...
	_GUICtrlListView_InsertColumn($hListView, 0, "文件名", 170)
	_GUICtrlListView_InsertColumn($hListView, 1, "文件大小(字节)", 100)
	_GUICtrlListView_InsertColumn($hListView, 2, "压缩后大小(字节)", 130)
	_GUICtrlListView_InsertColumn($hListView, 3, "优化比率 (%)", 80)
...
Func _Credits()
	...
	_GUICtrlRichEdit_AppendText($hRichEdit, "{\rtf1\utf8{\colortbl;\red128\green128\blue255;}\cf1 {\fs32 {\b   程序编写}}\cf0 \line \line }")
	Sleep(1000)
	_GUICtrlRichEdit_AppendText($hRichEdit, $binRtf)
	_GUICtrlRichEdit_AppendText($hRichEdit, @LF)
	Sleep($sleep)
	_GUICtrlRichEdit_AppendText($hRichEdit, "{\rtf1\utf8{\colortbl;\red128\green128\blue255;}\cf1 {\fs32 {\b   程序汉化 By 虫子樱桃}}\cf0 \line \line }")
	Sleep(1000)
	_GUICtrlRichEdit_AppendText($hRichEdit, "{\rtf1\utf8{\colortbl;\red0\green128\blue0;}\cf1 {\fs28 {\i  感谢:}}\cf0 \line \line }")
	Sleep(600)
	_GUICtrlRichEdit_SetFont($hRichEdit, 10 * $aDPI[0], "微软雅黑")
	Local $text = "工具使用了_Base64Encode() / LZMAT() / MsgBoxEx() 函数." & @LF & @LF & _
			"感谢网友trancexx的LZNTCompress / LZNTDecompress 和 _WinAPI_Base64Decode()函数!" & @LF & @LF &  _
			"感谢网友wraithdu对程序提出的BUG修复及相关建议." & @LF & @LF & @LF & _
			"按ESC键返回代码界面!"
...
Func WM_DROPFILES($hwnd, $msg, $wParam, $lParam)
	...
	If @error Or Not $aRet[0] Then Return SetError(1, 0, MsgBox(16 + 65536 + 262144, "错误", "在接受拖放文件时发生未知错误!", 10) * 0)
	...
	If $aRet[0] = 1 Then
		...
		If StringInStr(FileGetAttrib($sDroppedFiles), "D") Then
			...
			Return MsgBox(16 + 262144, "错误", "暂不支持文件夹!", 20, $hGUI)
		EndIf
	Else
		For $i = 0 To $aRet[0] - 1
		...
		If Not $j Then
			DllCall($hDll_shell32, "none", "DragFinish", "handle", $wParam)
			Return MsgBox(64 + 262144, "警告", "未添加任何文件!", 20, $hGUI)
		EndIf

		$sDroppedFiles = $sPrefix & $sDroppedFiles

		If $multipath Then MsgBox(64 + 262144, "警告", "当前暂不支持多个路径下的文件" & @CRLF & _
				"只添加了第一个路径下的文件!", 20, $hGUI)
	...
	Switch $wParam
		...
		Case $id_Copy
			...
			If $marked_txt = -1 Then
				...
				If $marked_txt = "" Or $marked_txt = "准备就绪." Then
					$e = True
				Else
					ClipPut($marked_txt)
				EndIf
