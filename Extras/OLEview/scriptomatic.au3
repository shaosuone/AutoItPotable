...
#AutoIt3Wrapper_Res_Comment=虫子樱桃自用汉化版 WMI 浏览(脚本生成)器
#AutoIt3Wrapper_Res_Description=虫子樱桃自用汉化版 WMI 浏览(脚本生成)器
#AutoIt3Wrapper_Res_Fileversion=1.0.0.10
#AutoIt3Wrapper_Res_LegalCopyright=Modified By虫子樱桃
...
GUICreate("虫子樱桃自用汉化版 AutoIt Scriptomatic V"&FileGetVersion(@ScriptFullPath), 684, 561, (@DesktopWidth - 684) / 2, (@DesktopHeight - 561) / 2, $WS_OVERLAPPEDWINDOW + $WS_VISIBLE + $WS_CLIPSIBLINGS,BitOR($WS_EX_TOOLWINDOW,$WS_EX_WINDOWEDGE))
...
Global $GUI_WMI_Namespace = GUICtrlCreateCombo("WMI_Namespaces", 20, 50, 280, 80,BitOR($GUI_SS_DEFAULT_COMBO,$CBS_SIMPLE))
GUICtrlSetOnEvent(-1,'HandleNamespaceChange')
...
GUICtrlSetOnEvent(-1,'HandleOutputChange')
...
Global $OutputHotKey = GUICtrlCreateCheckbox("不进行即时输出(低配置计算机适用)", 8, 544, 337, 17)
GUISetState()
...
Func _Edit_Changed()
	_GUICtrlComboBox_AutoComplete($GUI_WMI_Classes)
	If GUICtrlRead($OutputHotKey)=$GUI_UNCHECKED Then
	  ComposeCode()
	EndIf
...
Func WM_COMMAND($hWnd, $iMsg, $wParam, $lParam)
	...
	If Not IsHWnd($GUI_WMI_Classes) Then $hWndCombo = GUICtrlGetHandle($GUI_WMI_Classes)
	$hWndFrom = $lParam
	$iIDFrom = BitAND($wParam, 0xFFFF) ; Low Word
	$iCode = BitShift($wParam, 16) ; Hi Word
	Switch $hWndFrom
		Case $GUI_WMI_Classes, $hWndCombo
			Switch $iCode
...
EndFunc
Func _CodeToClip()
	Local $data = GUICtrlRead($GUI_AST_ScriptCode)
	If $data = '' Or $data = '载入中,请等待一会儿...' Then
		MsgBox(0,'提示','请生成代码后再进行复制~~',5)
	Else
		ClipPut($data)
		MsgBox(0,'','已经复制所生成代码到剪切板',5)
	EndIf
EndFunc
;********************************************************************
;* LoadWMINamespaces
...
Func ComposeCode()
	...
	If $strSelectedClass <> "选择一个 WMI 类('class')" Then
		...
		Local $objWMIService = ObjGet("winmgmts:{impersonationLevel=impersonate}!\\" & @ComputerName & $g_strCurrentNamespace),$objClass = $objWMIService.Get($strSelectedClass),$strScriptCode = ""

		If IsObj($objClass) Then
			$strScriptCode &= "; 生成于虫子樱桃自用汉化版 AutoIt Scriptomatic" & @CRLF & @CRLF
			...
			If $g_strOutputFormat = "Dialog" Then
				$strScriptCode &= '	      if Msgbox(1,"WMI输出",$Output) = 2 then ExitLoop' & @CRLF
...
Func SetWMIRepository()
	...
	Local $strWMISourceName = InputBox("设置WMI源", _
			"请输入您读取WMI信息的计算机名称: ", _
			$g_strWMISource)
