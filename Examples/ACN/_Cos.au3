...
Func _Main()
	...
	Local $aPoint[702][2], $j = 1, $hDashPen

	; 创建界面
	$hGUI = GUICreate("Cosine", 400, 300)
	$hWnd = WinGetHandle("Cosine")
	GUISetState()
	...
	_GDIPlus_GraphicsDrawStringEx($hGraphic, 'Cosine', $hFont, $tLayout, $hFormat, $hBrush)
	$tLayout = _GDIPlus_RectFCreate(279, 150, 50, 20)
	_GDIPlus_GraphicsDrawStringEx($hGraphic, 'π/2', $hFont, $tLayout, $hFormat, $hBrush)
	$tLayout = _GDIPlus_RectFCreate(350, 150, 50, 20)
	_GDIPlus_GraphicsDrawStringEx($hGraphic, 'π', $hFont, $tLayout, $hFormat, $hBrush)
	$tLayout = _GDIPlus_RectFCreate(200, 100, 20, 20)
	_GDIPlus_GraphicsDrawStringEx($hGraphic, '1', $hFont, $tLayout, $hFormat, $hBrush)
	$tLayout = _GDIPlus_RectFCreate(200, 200, 20, 20)
	_GDIPlus_GraphicsDrawStringEx($hGraphic, '-1', $hFont, $tLayout, $hFormat, $hBrush)
	...
	_GDIPlus_GraphicsDrawLine($hGraphic, 0, 100, 400, 100, $hDashPen)
	_GDIPlus_GraphicsDrawLine($hGraphic, 279, 0, 279, 300, $hDashPen)
	_GDIPlus_GraphicsDrawLine($hGraphic, 0, 200, 400, 200, $hDashPen)
	_GDIPlus_GraphicsDrawLine($hGraphic, 358, 0, 358, 300, $hDashPen)


	$aPoint[0][0] = 701
	For $i = -3.5 To 3.5 Step 0.01
		$aPoint[$j][0] = $i * 50 + 200
		$aPoint[$j][1] = 150 - _Cos($i) * 50
		$j += 1
	Next
