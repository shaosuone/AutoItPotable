...
Func _Main()
  Local $hGUI, $hWnd, $hGraphic, $hBrush, $tLayout, $hFormat, $hFamily, $hFont, $tLayout, $hPen
  Local $aPoint[802][2], $j = 1

  ; 创建界面
  $hGUI = GUICreate("Versin", 400, 300)
  $hWnd = WinGetHandle("Versin")
  GUISetState()
  ...
  _GDIPlus_GraphicsDrawStringEx($hGraphic, 'Versin', $hFont, $tLayout, $hFormat, $hBrush)
  ...
  For $i = -4 To 4 Step 0.01
    $aPoint[$j][0] = $i * 50 + 200
    $aPoint[$j][1] = 150 - _Versin($i * $pi) * 50
    $j += 1
  Next
