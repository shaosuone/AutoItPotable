...
Func _Main()
	...
	; 创建界面
	$hGUI = GUICreate("Inverse Haversin", 400, 300)
	$hWnd = WinGetHandle("Inverse Haversin")
	GUISetState()
	...
	_GDIPlus_GraphicsDrawStringEx($hGraphic, 'Inverse Haversin', $hFont, $tLayout, $hFormat, $hBrush)
	...
	For $i = -4 To 4 Step 0.01
		$aPoint[$j][0] = $i * 50 + 200
		$aPoint[$j][1] = 150 - _AHaversin($i) * 50
		$j += 1
	Next
