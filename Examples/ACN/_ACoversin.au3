...
Func _Main()
	...
	Local $aPoint[201][2], $j = 1, $hDashPen

	; 创建界面
	$hGUI = GUICreate("ACoversin", 400, 300)
	$hWnd = WinGetHandle("ACoversin")
	GUISetState()
	...
	_GDIPlus_GraphicsDrawStringEx($hGraphic, 'ACoversin', $hFont, $tLayout1, $hFormat, $hBrush)
	$tLayout2 = _GDIPlus_RectFCreate(300, 150, 10, 20)
	_GDIPlus_GraphicsDrawStringEx($hGraphic, '2', $hFont, $tLayout2, $hFormat, $hBrush)
	...
	_GDIPlus_GraphicsDrawLine($hGraphic, 300, 0, 300, 300, $hDashPen)

	$aPoint[0][0] = 200
	For $i = 0 To 2 Step 0.01
		$aPoint[$j][0] = $i * 50 + 200
		$aPoint[$j][1] = 150 - _ACoversin($i) * 50
		$j += 1
	Next
